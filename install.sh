#!/bin/sh
# install.sh — install datapact from GitHub.
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/datapact/main/install.sh | sh
#
# Installs the `datapact` binary globally with Bun. Pass a git ref as the
# first argument to pin a branch, tag, or commit:
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/datapact/main/install.sh | sh -s v0.1.0
set -eu

REPO="srivtx/datapact"
BIN="datapact"
REF="${1:-main}"
